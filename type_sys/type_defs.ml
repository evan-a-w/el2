...
let rec type_of_type_expr type_expr : mono =
  let open State.Result.Let_syntax in
  match type_expr with
  | Ast.Type_expr.Pointer type_expr -> Reference (type_of_type_expr type_expr)
  | Ast.Type_expr.Tuple l -> Tuple (List.map l ~f:type_of_type_expr)
  | Arrow (first, second) ->
    let first = type_of_type_expr first
    and second = type_of_type_expr second in
    Closure (first, second, { closed_vars = [] })
  | Closure (a, (_ : string), b) ->
    let first = type_of_type_expr a
