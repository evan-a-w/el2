...
let lookup_binding_id binding_id =
  ...
  match Binding_id.Map.find state.binding_map binding_id with
...
and show_mono mono =
  ...
  | Closure (a, b, binding_map) ->
    let%bind list =
      Binding_id.Set.fold binding_map ~init:(return []) ~f:(fun acc id ->
        let%bind acc = acc in
...
let add_field field poly =
  ...
  match
    Lowercase.Map.add
      state.current_module_binding.toplevel_fields
      ~key:field
      ~data:poly
  with
  | `Ok toplevel_fields ->
        ...
        State.Result.error
          [%message
            "type definition has free type variables"
              (user_type : user_type)
              ~vars:
                (Lowercase.Map.to_alist type_proof.tyvar_map |> List.map ~f:fst
                  : Lowercase.t list)
...
let gen_var var ~(add_var : add_var_t) ~pop_var =
  let open State.Result.Let_syntax in
  let%bind value, binding_id = lookup_var (Qualified.Unqualified var) in
...
and already_bound_node ~acc ~shadowed (node : Ast.node) =
  ...
  | Ast.Var (Qualified.Unqualified s) ->
    ...
    (match%map lookup_local_var s with
     | Some _ when not is_shadowed -> Binding_id.Set.add acc binding_id
     | _ -> acc)
...
let rec type_node node =
  ...
  | Ast.Var var_name ->
    let%bind poly, binding_id = lookup_var var_name in
    ...
    let%bind expr_map =
      Lowercase.Map.fold
        ...
        let%map () =
          ...
          | None ->
            State.Result.error [%message "Unknown field" (field : Lowercase.t)]
    ...
    let%bind inner_map, vars_list =
      Lowercase.Map.fold
        ...
        ~f:(fun ~key ~data:(binding, poly) acc ->
          ...
          in
          let%map () = unify mono' mono in
          let inner_map = Lowercase.Map.set inner_map ~key ~data:inner in
...
and type_expr expr =
  ...
  | Let_in (Ast.Nonrec (binding, expr1), expr2) ->
    let%bind binding, expr, vars =
      add_nonrec_bindings ~binding ~expr:expr1 ~add_var_mono:add_local_var_mono
    in
  ...
  | Let_in (Ast.Rec l, expr2) ->
    let%bind bindings, exprs, vars =
      add_rec_bindings ~add_var:add_local_var ~pop_var:pop_local_var l
    in
      ...
      State.Result.all
        (List.map cases ~f:(fun (binding, expr2) ->
           let_each_of_binding_typed
             ...
             ~add_var_mono:add_local_var_mono
